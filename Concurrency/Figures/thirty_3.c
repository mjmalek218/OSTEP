...
void thr_exit()
{
  ...
  /* So now this all makes sense...while the */
...
void thr_exit()
{
  ...
  /* Notice the lock is acquired here *before* calling the signal. This is to
     ensure the updating of the counting associated with the condition variable
     is performed atomically along with the signaling */
  Pthread_mutex_lock(&m);
  done = 1;
  Pthread_cond_signal(&c);
  Pthread_mutex_unlock(&m); /* This implies the original thread *still* has
                               the lock somehow...u fkin wot m8.
                               also the pintos code implies here that the lock
                               must be held for this operation to be called. */
}
